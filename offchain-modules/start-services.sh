...
then
  ...
  ${CKB_CLI} tx build-multisig-address --sighash-address ckt1qyqyph8v9mclls35p6snlaxajeca97tc062sa5gahk ckt1qyqvsv5240xeh85wvnau2eky8pwrhh4jr8ts8vyj37 --threshold 2 > ${FORCE_LOG_PATH}/mutisig.log
  lock_args=`cat ${FORCE_LOG_PATH}/mutisig.log| awk '{print $2}'| awk 'NR==1' `
  echo $lock_args
  address=`cat ${FORCE_LOG_PATH}/mutisig.log| awk '{print $2}'| awk 'NR==4' `
  echo $address
