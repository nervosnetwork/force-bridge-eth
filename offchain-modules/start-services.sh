...
then
  ...
  ${FORCE_CLI} ckb-relay -k 1 --per-amount 5  --max-tx-count 10 > ${FORCE_LOG_PATH}/ckb-relayer.log 2>&1 &
  ...
  ${FORCE_CLI} ckb-relay --network "${FORCE_NETWORK}" -k 1 --per-amount 5  --max-tx-count 10 > ${FORCE_LOG_PATH}/ckb-relayer.log 2>&1 &
