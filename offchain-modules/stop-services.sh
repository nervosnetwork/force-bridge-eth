...
ps aux | grep 'sign-server run' | grep -v grep | awk '{print $2}' | xargs kill -9
ps aux | grep 'dapp eth-header-indexer' | grep -v grep | awk '{print $2}' | xargs kill -9
ps aux | grep 'dapp ckb-header-indexer' | grep -v grep | awk '{print $2}' | xargs kill -9
