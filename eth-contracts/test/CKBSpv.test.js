  ...
  describe('proveTxExist correct case', async function () {
    ...
    it('Should proveTxExist verified', async () => {
      for (let i = 0; i < expectedTransactionsRoot.length; i++) {
        ...
        // mockForProveTxExist
        const res = await ckbChain.mockForProveTxExist(latestBlockNumber, blockNumber, blockHash, transactionsRoot)
        await waitingForGasUsed(provider, res)
        expect(await ckbChain.callStatic.latestBlockNumber()).to.equal(latestBlockNumber)
